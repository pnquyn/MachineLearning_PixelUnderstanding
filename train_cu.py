...
def train_one_epoch(model, loader, optimizer, criterions, device, loss_weights):
    ...
    # Biến để tính Global Dice Score
    total_intersection = 0.0
    total_pred = 0.0
    total_target = 0.0
    ...
    for context, target_img, target_label, _ in pbar:
        ...
        # Tính Loss
        # LƯU Ý: Nếu DiceLoss của bạn CHƯA có hàm sigmoid bên trong, 
        # hãy bọc pred_pixel bằng torch.sigmoid() ở dòng dưới đây:
        # loss_pixel = criterions['pixel'](torch.sigmoid(pred_pixel), target_img)
        loss_pixel = criterions['pixel'](pred_pixel, target_img)
        loss_cls = criterions['cls'](pred_cls, target_label)
        ...
        # Tính toán các thành phần cho Global Dice Score
        with torch.no_grad():
            pred_binary = (torch.sigmoid(pred_pixel) > 0.5).float()
            target_binary = (target_img > 0.5).float()
            ...
            # Tính Dice của batch hiện tại chỉ để hiển thị trên thanh tiến trình
            batch_dice = (2. * intersection + 1e-6) / (pred_sum + target_sum + 1e-6)

        total_loss += loss.item()

        # Cập nhật thanh tiến trình, hiển thị rõ px_loss và cls_loss để debug
        pbar.set_postfix(
            loss=f"{loss.item():.4f}", 
            px_loss=f"{loss_pixel.item():.4f}",
            cls_loss=f"{loss_cls.item():.4f}",
            b_dice=f"{batch_dice:.4f}"
        )

    # Tính Global Dice cho toàn bộ epoch
...
def validate(model, loader, criterions, device, loss_weights):
    ...
    # Tính Global Dice cho toàn bộ tập validation
    epoch_dice = (2. * total_intersection + 1e-6) / (total_pred + total_target + 1e-6)
    return total_loss / len(loader), epoch_dice
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        curr_lr = optimizer.param_groups[0]["lr"]
        print(f"Train Loss: {train_loss:.6f} | Val Loss: {val_loss:.6f} | LR: {curr_lr:.6f}")
        print(f"Train Global Dice: {train_acc:.4f} | Val Global Dice: {val_acc:.4f}")
        ...
        if val_acc > best_dice:
            best_dice = val_acc
            print(f"🔥 Đã tìm thấy Best Model mới với Global Dice: {best_dice:.4f}! Lưu tại {best_checkpoint}")
            torch.save(model.state_dict(), best_checkpoint)
